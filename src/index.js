...
function checksExistsUserTodo(request, response, next) {
  ...
  if (!todo)
    return response.status(404).json({ error: "This user todo not exists!" });
...
app.post("/users", (request, response) => {
  ...
  if (userAlreadyExists)
    return response.status(400).json({ error: "This user exists!" });
...
app.put("/users", checksExistsUserAccount, (request, response) => {
  const { user } = request;
